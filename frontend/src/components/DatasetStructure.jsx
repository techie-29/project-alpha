export default function DatasetStructure({ columns }) {
  return <section className="result-section" aria-labelledby="structure-title">
    ...
    {columns.length ? <div className="structure-table" role="table" aria-label="Extracted dataset columns">
      <div className="structure-row structure-head" role="row"><span role="columnheader">Column name</span><span role="columnheader">Detected type</span><span role="columnheader">Missing</span></div>
      {columns.map((column, index) => <div className="structure-row" role="row" key={`${column.name}-${index}`}><span role="cell"><i>{index + 1}</i>{column.name}</span><span role="cell">{column.detectedType || <em>Not reported</em>}</span><span role="cell">{column.missingCount ?? <em>—</em>}</span></div>)}
