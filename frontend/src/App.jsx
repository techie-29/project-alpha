...
export default function App() {
  ...
  useEffect(() => {
    ...
    getCurrentAccount(token).then((response) => {
      if (!active) return;
      setAccount(response.data.account); setAuthStatus("signed-in");
    }).catch(() => {
      if (!active) return;
      localStorage.removeItem(TOKEN_KEY); setToken(null); setAccount(null); setAuthStatus("signed-out");
    });
    return () => { active = false; };
  }, [token]);

  function handleAuthenticated(authData) {
    localStorage.setItem(TOKEN_KEY, authData.token); setToken(authData.token); setAccount(authData.account); setAuthStatus("signed-in");
  }
  function handleLogout() {
    localStorage.removeItem(TOKEN_KEY); setToken(null); setAccount(null); setAuthStatus("signed-out"); resetUpload();
  }
  function handleFileSelect(file) {
    const validationError = validateFile(file); setError(validationError); setResult(null);
    if (validationError) { setSelectedFile(null); setStatus("invalid"); return; }
    setSelectedFile(file); setStatus("selected");
  }
  ...
  if (account?.role === "admin") return <AdminPanel token={token} account={account} onLogout={handleLogout}/>;
